...
// Package oracle is a database adapter for oracle.
package oracle
...
import (
	...
	ms "github.com/go-sql-driver/oracle"
...
//     db, err := sql.Open("godror", "VT_SURGALT/surgalt1046pwd@192.168.100.46:1521/db11g")
//     if err != nil {
// 		fmt.Println(err)
...
//     rows,err := db.Query("select order_id from cafe_order_2020")
//     if err != nil {
...
const (
	defaultDSN      = "root:@tcp(192.168.100.46:1521)/tinode?parseTime=true"
...
func (a *adapter) Open(jsonconfig json.RawMessage) error {
	if a.db != nil {
		return errors.New("oracle adapter is already connected")
	}
	...
	if err = json.Unmarshal(jsonconfig, &config); err != nil {
		return errors.New("oracle adapter failed to parse config: " + err.Error())
	}
	...
	// a.db, err = sqlx.Open("oracle", a.dsn)
	a.db, err = sqlx.Open("godror", "VT_SURGALT/surgalt1046pwd@192.168.100.46:1521/db11g")
	if err != nil {
		return err
	}
...
func (a *adapter) CreateDb(reset bool) error {
	...
	a.db, err = sqlx.Open("oracle", cfg.FormatDSN())
	if err != nil {
		return err
	}
	...
	defer func() {
		if err != nil {
			// FIXME: This is useless: oracle auto-commits on every CREATE TABLE.
...
func (a *adapter) MessageGetAll(topic string, forUser t.Uid, opts *t.QueryOpt) ([]t.Message, error) {
	...
	if opts != nil {
		...
		if opts.Before > 0 {
			// oracle BETWEEN is inclusive-inclusive, Tinode API requires inclusive-exclusive, thus -1
			upper = opts.Before - 1
		}
	...
	if toDel == nil {
		...
		if err == nil && toDel.DeletedFor == "" {
			...
			if len(toDel.SeqIdRanges) > 1 || toDel.SeqIdRanges[0].Hi == 0 {
				...
				// oracle's BETWEEN is inclusive-inclusive thus decrement Hi by 1.
...
// Check if oracle error is a Error Code: 1062. Duplicate entry ... for key ...
func isDupe(err error) bool {
	if err == nil {
		return false
	}

	myerr, ok := err.(*ms.oracleError)
